 ...
 */
class ZooPlusReverseEntry extends Entry
{
    /**
     * {@inheritdoc}
     */
     ...
     * {@inheritdoc}
     */
    protected $collectionClass = '\\rsanchez\\Deep\\Collection\\ZooPlusReverseCollection';
        ...
        return array_merge(parent::joinTables(), [
            'zoo_plus_relationships' => function ($query) {
                $query->leftJoin('zoo_plus_relationships',function($join){
                    $join->on( 'zoo_plus_relationships.parent_id', '=', 'channel_titles.entry_id');
                });
            },
        ]);
    }
    ...
    public function scopeParentEntryId(Builder $query, $entryId)
    {
        ...
        return $this->requireTable($query, 'zoo_plus_relationships', true)->whereIn('zoo_plus_relationships.child_id', $entryId);
    }
}
